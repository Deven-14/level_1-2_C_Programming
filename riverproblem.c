...
void create_pairs(int i,Hobbit_team t[i],Hobbit_pairs p[])                                                      // don't pass by reference until data is getting manipulated in that function
{
...
void remove_hobbit_who_reached(int i,Hobbit_team *t_B,Hobbit_team t[i])
{
    int a,b;
    for(a=0;a<t[i].n;a++)
    {
        if(t_B->team_member[t_B->n].speed==t[i].team_member[a].speed)
            b=a;
    }
    for(a=b;a<t[i].n-1;a++)                                                                                        // a<t[i].n-1 coz a+1 is there
        t[i].team_member[a].speed=t[i].team_member[a+1].speed;
    t[i].n--;
}
void adding_hobbit_who_came_back(int i,int hob_return,Hobbit_team t[i])
{
...
void remove_hobbit_pair_which_reached(int i,Hobbit_team *t_B,int p_num,Hobbit_pairs p[],Hobbit_team t[i])
{
    t_B->team_member[t_B->n].speed=p[p_num].a.speed;                                                                 //starts from t_B.n=0
    remove_hobbit_who_reached(i,t_B,t);
    t_B->n++;                                                                                                        //makes this t_B.n=1
    t_B->team_member[t_B->n].speed=p[p_num].b.speed;
    remove_hobbit_who_reached(i,t_B,t);
}
int find_which_hobbit_returns(int i,Hobbit_team t[i],Hobbit_team *t_B)
{
    ...
    adding_hobbit_who_came_back(i,hobbit_returning,t);
    for(j=l;j<t_B->n;j++)
        t_B->team_member[j].speed=t_B->team_member[j+1].speed;
    t_B->n--;
    return hobbit_returning;
}
void put_back_all_hob_to_check_diff_cases(int i,Hobbit_team t[i],Hobbit_team *t_B)
{
    int a,b,temp;
    for(a=0;a<t_B->n-1;a++)
    ...
    for(a=0;a<4;a++)
        t[i].team_member[a].speed=t_B->team_member[a].speed;
    t[i].n=4;
    t_B->n=0;
}
...
void find_time_val(int i,Hobbit_team t[i],int time_values[3])
{
    ...
    {                                                                                                     // hob_r_pos is the position of the hobbit who reached the other side
        for(j=0;j<3;j++)
        {
            ...
            remove_hobbit_pair_which_reached(i,&t_B,p_num,p,t);                                           //p_num*********
            if(j!=2)
                b_crs[m+1]=find_which_hobbit_returns(i,t,&t_B);                                           //we use this instead of add_hob because add_hob func is called in find hob func
            m+=2;
            t_B.n++;
        } 
        put_back_all_hob_to_check_diff_cases(i,t,&t_B);                                                   // add all the hobbits back to try for different time values
        t_crs=b_crs[0]+b_crs[1]+b_crs[2]+b_crs[3]+b_crs[4];
        time_values[k]=t_crs;
        m=0;                                                                                              //  it should start over again
    }
}
void find_st(int i,Hobbit_team t[i])                                                                      // here t2 is a formal parameter which is a pointer, to avoid confusion u can write here Hobbit_team *ptr, therefore ptr will have the address of t2
{                                                                                                         //here t2 has to be passed by reference as value is getting changed or this function has to return t2, since it's not a array it won't be passed by reference automatically
    ...
    find_time_val(i,t,time_values);
    t[i].st_crs=time_values[0];
    for(m=0;m<3;m++)
    {
        if(time_values[m]<t[i].st_crs)
            t[i].st_crs=time_values[m];
    }
}
void find_st_all_teams(int n,Hobbit_team t[n],int st_all_teams[n])
{
	...
	for(i=0;i<n;i++)
	{
		find_st(i,t);                                                                                     //  IN COMMIT 28, core is getting dumped here, that's because we didn't assign the structure pointer an address
